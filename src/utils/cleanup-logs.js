  ...
  try {
    ...
    files.forEach((file) => {
      ...
      try {
        ...
        console.log(
          `File: ${file}, Age: ${fileAgeDays.toFixed(2)} days, Size: ${(stats.size / 1024).toFixed(2)} KB`,
        );
        ...
        if (fileAgeDays > MAX_AGE_DAYS) {
          try {
            ...
            deletedCount++;
            console.log(
              `Successfully deleted: ${file} (${Math.floor(fileAgeDays)} days old)`,
            );
          } catch (deleteErr) {
            errorCount++;
          ...
          console.log(
            `File ${file} not old enough for deletion (${fileAgeDays.toFixed(2)} days < ${MAX_AGE_DAYS} days)`,
          );
          skippedCount++;
        }
      } catch (statErr) {
    ...
    console.log(
      `Cleanup complete: ${deletedCount} deleted, ${errorCount} errors, ${skippedCount} skipped`,
    );
  } catch (err) {
